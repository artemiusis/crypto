...
async def get_token_prices():
    ...
    while True:
        ...
        for i in range(int(len(mints)/30.001)+1):
            ...
            if mint in mints_data:
                ...
                if len(volumes) > 2:
                    ...
                    if mint not in used_mints and price_growth/100 >  float(growth) / 100 and max_volume >= min_volume * (volume_growth + 100) / 100 and token_market_cap >= market_cap and token_volume >= volume and token_liquidity >= liquidity and token_boosts >= boosts:
                        ...
                        price = float(mints_data[mint]["price"])
                        time = datetime.utcnow()
                        used_mints.append(mint)
                        asyncio.create_task(check_signal(price, user_id, mint, name, time))
...
async def check_signal(price, user_id, mint, name, time):
    k =  db.check_signal_success(user_id, name)
    n = db.check_signal_unsuccess(user_id, name)
    while True:
        if not db.check_signal_status(user_id, name):
        ...
        if os.path.exists(MINTS_FILE):
            ...
            mints_data = {}
        if mint in mints_data:
            if price * 2 < float(mints_data[mint]["price"]):
                k +=1
                message = f"🚀 Сигнал {name} робочий ({mint}). Мінімум x2.\nСигнал спрацював {k}/{k+n} разів"
                await bot.send_message(user_id, message)
                db.update_signal_success(user_id, name)
                return
            if datetime.utcnow() > time + timedelta(hours=1):
                n+=1
                message = f"🚀 Сигнал {name} ({mint}) не спрацював протягом години.\nСигнал спрацював {k}/{k+n} разів"
                await bot.send_message(user_id, message)
                db.update_signal_unsuccess(user_id, name)
                return
        else:
            return
        await asyncio.sleep(90)  # Перевірка кожні 90 секунд
