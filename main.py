...
async def cmd_info(message: types.Message):
    ...
    Тепер перед виводом ми також скануємо дані і додаємо дублікатні токени
    у repeat_x (за логікою process_token).
    """
    try:
        data = await read_file(DATA_FILE)
    except FileNotFoundError:
    ...
    # додаємо дублікатні токени у repeat_x
    try:
        modified = add_duplicates_to_repeat(data)
        if modified:
            # намагаємося зберегти зміни
            retries = 0
            while retries < 5:
                try:
                    with open(DATA_FILE, "w", encoding="utf-8") as f:
        ...
        print("Помилка при додаванні дублікатів у repeat_x:", e)
